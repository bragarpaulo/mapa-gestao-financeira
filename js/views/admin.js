...
// Lista paginada genérica (busca + paginação). filtro(u,q)→bool; linha(u)→<tr>; onRow opcional.
function paginar(body, itens, { ph, cols, filtro, linha, wire }) {
  ...
  function draw() {
    ...
    body.innerHTML = `<input class="gc-search" placeholder="${esc(ph)}" value="${esc(q)}">
...
async function loadAssinantes(body) {
  const users = (await cloud.adminListUsers()).filter(u => !u.is_admin);
  paginar(body, users, {
    ...
    wire: (b) => b.querySelectorAll('[data-edit]').forEach(btn => btn.onclick = () => { const u = users.find(x => x.id === btn.dataset.edit); openSubscriber(u); }),
  });
}
...
async function loadSistema(body) {
  ...
  paginar(body, admins, {
    ...
    wire: (b) => {
      ...
      const bar = document.createElement('div'); bar.className = 'toolbar'; bar.style.cssText = 'gap:6px;margin-top:10px';
      bar.innerHTML = `<input id="ad-email" type="email" placeholder="e-mail do novo admin" style="width:200px"><input id="ad-pw" type="text" placeholder="senha (mín. 6)" style="width:140px"><button class="btn btn-sm btn-primary" id="ad-add">+ Criar admin</button>`;
      b.appendChild(bar);
      bar.querySelector('#ad-add').onclick = async () => { const email = bar.querySelector('#ad-email').value.trim(), pw = bar.querySelector('#ad-pw').value; if (!email || pw.length < 6) { alert('E-mail + senha (mín. 6).'); return; } const r = await cloud.adminCreateAdmin(email, pw); if (r && r.ok) loadSistema(body); else alert('Erro: ' + (r && r.error || '')); };
    },
  });
}
...
async function loadPlans(body) {
  ...
  function draw() {
    ...
    body.innerHTML = `<p class="hint" style="margin:0 0 8px">"Oferta Green" liga a compra ao plano · "Seats" = membros de equipe.</p>
...
async function loadTemplates(body) {
  ...
  function draw() {
    ...
    body.innerHTML = `<input id="tp-q" class="gc-search" placeholder="🔎 Buscar template…" value="${esc(q)}">
      ...
      <div class="toolbar" style="margin-top:8px;gap:6px"><input id="tp-new-id" type="text" placeholder="slug (ex.: advogado)" style="width:140px"><input id="tp-new-nome" type="text" placeholder="Nome" style="min-width:140px"><button class="btn btn-sm" id="tp-add">+ Adicionar</button></div>`;
    ...
    body.querySelector('#tp-add').onclick = async () => { const id = (body.querySelector('#tp-new-id').value || '').trim().toLowerCase().replace(/\s+/g, '-'), nome = (body.querySelector('#tp-new-nome').value || '').trim(); if (!id || !nome) { alert('Informe slug e nome.'); return; } if (await cloud.adminUpsertTemplate({ id, nome, active: true })) loadTemplates(body); };
  }
  draw();
}
